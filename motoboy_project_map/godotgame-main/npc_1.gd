...
func _on_interact():
	if not GameState.tutorial_concluido:
		if sprite.frame == 0: 
			...
			if sprite.frame == 0:
				sprite.frame = 1
				textbox.show_text_box("???: Parabéns!,forasteiro vejo que você sobreviveu aos perigos da floresta!")
				await textbox.text_box_finished
				textbox.show_text_box("Motoboi: Quem é tu maluco , ta me estranhando ?!")
				await textbox.text_box_finished
				textbox.show_text_box("???: Perdão forasteiro!,me chamo Land!, guardião da floresta pantanosa")
				await textbox.text_box_finished
				textbox.show_text_box("Land: E você forasteiro como se chama ?! ")
				await textbox.text_box_finished
				textbox.show_text_box("Motoboi: Meu vulgo é motoboi, cria de itaquera parceiro!")
				await textbox.text_box_finished
				textbox.show_text_box("Land: não conheço esse reino, deve ser distante!")
				await textbox.text_box_finished
				textbox.show_text_box("Land: Enfim motoboi!, a frente está o lugar mais profundo dessa floresta!")
				await textbox.text_box_finished
				textbox.show_text_box("Land: Onde muitos aventureiros morreram pra chegar! e ninguém conseguiu realizar o feito!")
				await textbox.text_box_finished
				textbox.show_text_box("Land: A sua frente está o santuário da lendária Mochila!")
				await textbox.text_box_finished
				textbox.show_text_box("Land: Onde nenhum guerreiro foi forte o suficiente para vincular sua alma a energia da mochila")
				await textbox.text_box_finished
				textbox.show_text_box("Land: Todos os que tentaram tiveram suas almas perdidas pela mochila e assim morreram!")
				await textbox.text_box_finished
				textbox.show_text_box("Land: Ao lado do santuário está a saída da floresta e o caminho a grande cidadela!")
				await textbox.text_box_finished
				textbox.show_text_box("Land: Será você um herdeiro do antigo portador da mochila ?")
				await textbox.text_box_finished
				textbox.show_text_box("Motoboi: Que papo torto mano!")
				await textbox.text_box_finished
				textbox.show_text_box("Motoboi: Vamo ver qualé dessa mochila!")
				await textbox.text_box_finished
